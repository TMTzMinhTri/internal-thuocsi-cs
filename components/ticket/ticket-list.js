...
const TicketList = ({ total, tickets, reasonList, filter = {}, isMyTicket = false }) => {
  ...
  filter.reasonsFilter =
    filter?.reasons && reasonList.filter((item) => filter.reasons.indexOf(item.value) >= 0);
  ...
  const onSearch = useCallback(
    async ({ saleOrderCode, saleOrderID = 0, status, reasons, assignUser, fromTime, toTime }) => {
      ...
      <div className={styles.grid}>
        <MyCard>
          <MyCardHeader title="Danh sách phiếu hỗ trợ">
            ...
            >
              <FontAwesomeIcon icon={faCog} style={{ marginRight: 8 }} /> Thay đổi trạng thái hàng
              loạt
            </Button>
          </MyCardHeader>
          <form>
