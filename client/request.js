...
class RequestClient extends APIClient {
  ...
  getProductByRequestID(productID) {
    return this.callFromNextJS('GET', `${prefix}/product`, {
      productID: productID,
    });
  }
  ...
  getIngredientsByRequestCode(code) {
    return this.callFromNextJS('GET', `${prefix}/product/ingredient/list`, {
      productCode: code,
    });
  }

  getIngredientByCode(code) {
    return this.callFromNextJS('GET', `${prefix}/ingredient`, {
      ingredientCode: code,
    });
  }

  getListIngredients() {
    return this.callFromNextJS('GET', `${prefix}/ingredient/list`);
  }

  searchIngredients(q) {
    return this.callFromClient('GET', `${prefix}/ingredient/list?q=${q}`);
  }

  createNewRequest(data) {
    return this.callFromClient('POST', `${prefix}/product`, data);
  }

  updateRequest(data) {
    return this.callFromClient('PUT', `${prefix}/product`, data);
  }

  addIngredient({ ingredientCode, ingredientVolume, productCode }) {
    return this.callFromClient('POST', `${prefix}/product/ingredient`, {
      ingredientCode,
      ingredientVolume,
      productCode,
    });
  ...
  putUploadImage(data) {
    return this.client.makeRequest('PUT', `${prefix}/upload`, data);
  }
}
